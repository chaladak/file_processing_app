pipeline {
            ...
            steps {
                container('docker') {
                    script {    
                        ...
                        sh "docker build --network=host -t ${DOCKER_REGISTRY}/${PROJECT_NAME}-${SERVICE_NAME}:${TAG} ./notification_service"
                    }
                }
            }
        }
                ...
                container('docker') {
                    script {
                        ...
                        sh """
                            ...
                            cat infra/notifier.yaml | sed 's|\\\${DOCKER_REGISTRY}|${DOCKER_REGISTRY}|g' | sed 's|\\\${PROJECT_NAME}|${PROJECT_NAME}|g' | sed 's|\\\${TAG}|${TAG}|g' | kubectl apply -f -
                            kubectl -n ${PROJECT_NAME} wait --for=condition=available deployment/notifier --timeout=300s
                        """
                    }
                }
            }
    ...
    post {
        ...
        success {
            // Notify on success
            echo "Notifier service deployment completed successfully!"
            echo "Deployed ${DOCKER_REGISTRY}/${PROJECT_NAME}-${SERVICE_NAME}:${TAG}"
        }
        
        failure {
            // Notify on failure
            echo "Notifier service deployment failed!"
        }
    }
}
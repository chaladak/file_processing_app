            ...
            steps {
                container('docker') {
                    script {    
                        ...
                        sh "docker build --network=host -t ${DOCKER_REGISTRY}/${PROJECT_NAME}-${SERVICE_NAME}:${TAG} ./api_service"
                    }
                }
            }
        }
                ...
                container('docker') {
                    script {
                        ...
                        sh """
                            ...
                            cat infra/api.yaml | sed 's|\\\${DOCKER_REGISTRY}|${DOCKER_REGISTRY}|g' | sed 's|\\\${PROJECT_NAME}|${PROJECT_NAME}|g' | sed 's|\\\${TAG}|${TAG}|g' | kubectl apply -f -
                            kubectl -n ${PROJECT_NAME} wait --for=condition=available deployment/api --timeout=300s
                        """
                    }
                }
            }
